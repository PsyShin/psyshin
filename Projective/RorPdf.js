function generateRep(){
	...
	for(let i=0; i<5; i++){
		...
		for(let j=0; j<3; j++) FQCol_Pdf[j*5+i].innerHTML = FQCol_Ele[j+i*3].innerHTML;
	}
	...
	for(let i=1; i<BlendsCol_Ele.length; i++) BlendsCol_Pdf[i].innerHTML = BlendsCol_Ele[i].innerHTML;
}